...
Se o PowerShell não arrancar, usa diretamente `python app_server.py --host 127.0.0.1 --port 8765`; o próprio Python pede a password de forma oculta. O `run-ui.bat` continua disponível para Windows e tenta iniciar o launcher com política temporária de execução.
